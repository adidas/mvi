...
public class IntentDslScope<StateT : LoggableState, SideEffectT : Any>(
    ...
    public suspend fun reduce(block: (StateT) -> StateT) {
        val transform = object : ViewTransform<StateT, SideEffectT>() {
    ...
    public suspend fun postSideEffect(effect: SideEffectT) {
        val transform = object : SideEffectTransform<StateT, SideEffectT>() {
            override fun mutate(sideEffects: SideEffects<SideEffectT>): SideEffects<SideEffectT> =
    ...
    public suspend inline fun <reified SpecificStateT : StateT> reduceInState(
        noinline reducer: (SpecificStateT) -> StateT,
    ) {
        val transform = object : ViewTransform<StateT, SideEffectT>() {
